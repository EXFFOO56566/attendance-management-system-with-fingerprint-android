...
public class OutTimeActivity extends BaseActivity implements LocationManagerInterface {
    ...
    private TextView textViewName, titleName, dateName, info, location, infoTitle;

    String name, date, out_time, command;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // Init dialog
        dialogMessage = new ProgressDialog(OutTimeActivity.this);
        ...
        dateName = findViewById(R.id.tanggal);
    ...
    public void sendDataAttendance() {
        runOnUiThread(new Runnable() {
            ...
            public void run() {
                ...
                command = "out";
                date = today;
                out_time = currentDateTimeString;
                ...
                new SendOTPTask().execute(keyload, command, name, date, out_time, strAdd);
            }
        });
    }
        ...
        protected Void doInBackground(String... str) {
            ...
            try {
                ...
                if (!response.isSuccessful()) {
                    displayToastMessage("setText", getString(R.string.error_unexpected) + response, 0);
        ...
        Runnable doDisplayError = new Runnable() {
            public void run() {
                if (type.equals("text")) {
                    ...
                    // Save status
                    PrefUtils.saveToPrefs(mContext, PrefKeys.STATUS_CHECK, "CHECK-OUT");
                    dbManager.insertAttendance(userName, date, out_time, "Check-Out", strAdd);
                }
                ...
                if (mLocationManager.isGetAccuracyInfo()) {
                    if (dialogMessage!=null && dialogMessage.isShowing()) {
                        ...
                        if (isNetworkAvailable(true)) {
                            Toast.makeText(OutTimeActivity.this, getString(R.string.no_internet), Toast.LENGTH_LONG).show();
                        } else {
    ...
    public void checkInArea(boolean checkArea) {
        if (checkArea) {
            ...
            showDialogOutsideOffice(OutTimeActivity.this);
        }
    }
    ...
    public void checkUrlKeyName() {
        ...
        if (dbManager.checkIfEmpty("DATASABSENT")) {
            Toast.makeText(OutTimeActivity.this, getString(R.string.please_setting_url_key), Toast.LENGTH_LONG).show();
            ...
            if (linkload.equals("") && keyload.equals("")) {
                ...
                Toast.makeText(OutTimeActivity.this, getString(R.string.empty_name), Toast.LENGTH_LONG).show();
            } else if (idNumber.equals("")) {
                Toast.makeText(OutTimeActivity.this, getString(R.string.id_number_is_empty), Toast.LENGTH_LONG).show();
            } else if (!linkload.equals("") && !keyload.equals("")) {
                // Show popup search current location
                showPopupLocationHandler();
            } else if (keyload.equals("")) {
                Toast.makeText(OutTimeActivity.this, getString(R.string.setting_key), Toast.LENGTH_LONG).show();
            } else {
                Toast.makeText(OutTimeActivity.this, getString(R.string.setting_url), Toast.LENGTH_LONG).show();
            }
        }
    }
